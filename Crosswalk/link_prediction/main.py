...
# run w synth2 after 
def run_link_prediction(dataset, emb_type, rwl, bndrytype, bndry, exp, d, all_labels, n_iters=6, test_ratio=0.5):
    ...
    for iter in tqdm([str(k) for k in range(1,n_iters)]):
        ...
        if emb_type == 'unweighted':
            ...
            test_links_filepath = f'{filename}/{emb_type}/links/{emb_type}_{dataset}_d{d}-test-{test_ratio}.links'
            train_links_filepath = f'{filename}/{emb_type}/links/{emb_type}_{dataset}_d{d}-train-{test_ratio}.links'
        else:
            full_method=f'{emb_type}_{rwl}_{bndrytype}_{bndry}_exp_{exp}'
            test_links_filepath = f'{filename}/{emb_type}/links/{emb_type}_{rwl}_bndry_{bndry}_exp_{exp}-{dataset}-test-{test_ratio}.links'
            train_links_filepath = f'{filename}/{emb_type}/links/{emb_type}_{rwl}_bndry_{bndry}_exp_{exp}-{dataset}-train-{1-test_ratio}.links'
        
        emb_filepath = f'{filename}/{emb_type}/embeddings/{dataset}.embeddings_{full_method}_d{d}'
    ...
    return (avg_accuracies, {'embedding_type':emb_type, 'rwl':rwl, 'boundary_type':bndrytype, 'boundary_val':bndry, 'exp':exp, 'd': d})

def experiment_parameters(dataset, emb_type, r, bndrytype, bndry, exp, d, all_labels, threads=0, test_ratio=0.5):
    if threads <= 1:
        accuracies = []
                ...
                for bndry in bndries:
                    for exp in exponents:
                        for bndrytype in bndry_types:
                            for d in ds:
                                accuracy, args= run_link_prediction(dataset, emb_type, r, bndrytype, bndry, exp, d, all_labels, test_ratio=test_ratio)
            ...
            for r in rwl:
                for bndry in bndries:
                    for exp in exponents:
                        for bndrytype in bndry_types:
                            for d in ds:
                                input_args.append((dataset, emb_type, r, bndrytype, bndry, exp, d, all_labels))
...
if __name__ == '__main__':
    ...
    ds = [32, 64, 92, 128] #128, 32, 64, 92
    all_labels = [0, 1] if dataset == 'rice_subset' else [0, 1, 2]
    ...
    test_ratio = 0.5
    accuracies = experiment_parameters(dataset, emb_types, rwl, bndry_types, bndries, exponents, ds, all_labels, threads=24, test_ratio=test_ratio)
    df = pd.DataFrame.from_dict(accuracies)
    print(df)
    df.to_csv(f'{dataset}_link_prediction_results_test_ratio_{test_ratio}.csv')
    # bias parameters
    # p default 1
    # alpha = bndries